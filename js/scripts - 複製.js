    ...
    var handleTablesNanomark = function () {
		...
		var table_specimen_facility_booking_list = $("#table_specimen_facility_booking_list").dataTable({
			...
			"fnServerParams": function ( aoData ) {
				aoData.push({"name":"specimen_ID","value":$(this).find("#specimen_ID").val()});
	        },
		});
			...
			"aoColumnDefs": [ 
				{
				  ...
				  "mData": null,
			      "mRender": function ( data, type, full ) {
			        return '<a href="/index.php/admin/edit/'+full[0]+'" class="btn btn-warning">編輯</a>';
			      }
			    }
			],
        });
...
//					passwd:{required:true,minlength:8},
				...
				messages:{
					ID:{required:'請輸入ID',minlength:'不可小於4個位元'},
					name:{required:'請輸入您的姓名'},
//					passwd:{required:'請輸入您自行設定的密碼',minlength:'不可小於8個位元'},
					passwd2:{equalTo:'密碼不同'},
					mobile:{required:'請輸入您的行動電話'},
	...
	var handleTablesFacility = function(){
		...
		table_facility_booking_available_time = $("#table_facility_booking_available_time").dataTable({
			...
			"fnServerParams": function ( aoData ) {
				aoData.push({"name":"facility_ID","value":$("input[name='facility_ID'],select[name='facility_ID']").val()},
							{"name":"booking_ID","value":$("input[name='booking_ID']").val()},
	        ...
	        "fnDrawCallback": function( oSettings ) {
		        ...
		        if (test = $(this).find("input[type=checkbox]:not(.toggle), input[type=radio]:not(.toggle)")) {
		            test.uniform();
		        }
		    },
        ...
        $("#form_facility_booking input[name='purpose']").click(function(){
        	...
        	{
				$("#user_selector").hide();
				$("#form_facility_booking select[name='user_ID']").prop('selectedIndex',0);
			}else{
				$("#user_selector").show();
			}
        });
		...
		$("#synchronize_access_card").click(function(){
			$.ajax({
			  url: "/index.php/facility/admin/access/card/synchronize/",
			}).always(function(data) {
			  access_card_table.fnReloadAjax();
			});
		});
		//-----------------------卡機連線列表------------------------------
	...
	var handleTablesCurriculum = function(){
        ...
        var table_curriculum_class_list = $("#table_curriculum_class_list").dataTable({
			...
			"aaSorting": [],
			"fnServerParams": function ( aoData ) {
		      aoData.push(  {"name": "class_code", "value": $("#class_code").val() } );
		    }
        });
        ...
        $("#query_curriculum_reg_list").click(function(){
        	table_curriculum_reg_list.fnReloadAjax(null,null,true);
        });
        //lesson
	...
	var handleFormNanomarkOutsourcing = function() {
		...
		//表單驗證
		$('#form_nanomark_outsourcing').validate
		(
			{
				rules:{
					verification_norm_name:{required:true},
					outsourcing_organization:{required:true},
					"test_items_name_1[]":{required:true,minlength:1},
					"test_items_name_2[]":{required:true,minlength:1},
					"test_items_name_3[]":{required:true,minlength:1},
				},
				messages:{
	...
	var handleFormNanomarkCustomerSurvey = function(){
		//表單驗證
		$validator = $('#form_customer_survey').validate
		(
			{
				rules:{
					overall_quality:{required:true},
					request_form:{required:true},
					response_question:{required:true},
					communication:{required:true},
					time_test:{required:true},
